...
class GPT(nn.Module):
    ...
    def assign_kv_caches(self, kv_caches: List[KVCache]):
        ...
        for cache, block in zip(kv_caches, self.transformer.h):
            self._check_kv_cache(self.config, cache)
            ...
            if not for_prefill:
                for l_ix, block in enumerate(self.transformer.h):
                    ...
                    if kv_cache.next_token_pos != input_pos:
                        raise ValueError(
                            f"KV cache for layer {l_ix}: input_pos = {input_pos} != {kv_cache.next_token_pos} = kv_cache.next_token_pos"
                        )
                    if kv_cache.max_tokens_forward < T:
                        raise ValueError(
                            f"KV cache for layer {l_ix}: T = {T}, must be <= max_tokens_forward = {kv_cache.max_tokens_forward}"
                        )
        ...
        for l_ix, block in enumerate(self.transformer.h):
            if for_prefill:
                ...
                if attn.kv_cache.batch_size < eff_batch_size:
                    raise ValueError(
                        f"Batch size {eff_batch_size} is too large for KV cache layer {l_ix} (batch size {attn.kv_cache.batch_size}). Use 'assign_kv_caches' or `set_kv_cache'"
                    )
            if hook is not None:
                # Call start of layer hook, passing detached layer input
                hook(x.detach(), l_ix)
            x = block(x, cos, sin, idx, self.mha, input_pos)
