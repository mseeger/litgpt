...
class DenseKVCache(DefaultKVCache):
    ...
    Note: This implementation reserves buffers up front, using
    `max_batch_size`. This could be improved.
    ...
    def __init__(
        ...
        **base_kwargs,
        ...
        Args:
            config: Model config
            max_batch_size: Inference batch size
        ...
        shape = (max_batch_size, self.n_query_groups, max_sequence_length, self.head_size)
    ...
    def get_keys_values(self) -> Optional[KeysAndValues]:
        if self.batch_size is None or self.next_position is None:
            return None
        else:
            return DefaultKeysAndValues(
                self.k[: self.batch_size, :, : self.next_position, :],
                self.v[: self.batch_size, :, : self.next_position, :],
            )
    ...
    def _forward(
        ...
        self.k[: self.batch_size, :, np: (np + num), :] = key
        self.v[: self.batch_size, :, np: (np + num), :] = value
    ...
    def _prefill(
        ...
        if batch_size > self.max_batch_size:
            raise ValueError(f"key.shape[0] = {batch_size} must be at most {self.max_batch_size}")
        ...
        self.k[:batch_size, :, :init_length, :] = key
        self.v[:batch_size, :, :init_length, :] = value
...
class LastRecentlyInsertedKVCache(DefaultKVCache):
    ...
    def __init__(
        ...
        **base_kwargs,
        ...
        shape = (max_batch_size, self.n_query_groups, cache_length, self.head_size)
        self.register_buffer("v", torch.zeros(shape, device=device, dtype=dtype), persistent=False)
        # TODO: Remove once HF bug fixed
        if self._work_around_hf_bug:
            shape = shape[:-1] + (self.head_size + 1,)
    ...
    def _forward(
        ...
        self.k[: self.batch_size, :, np: (np + num1), :] = key[:, :, :num1, :]
        self.v[: self.batch_size, :, np: (np + num1), :] = value[:, :, :num1, :]
        ntp = self._next_token_pos
        self.token_pos[np : (np + num1)] = torch.arange(ntp, ntp + num1, device=self.device, dtype=torch.int)
        if num1 < num:
            diff = num - num1
            self.k[: self.batch_size, :, :diff, :] = key[:, :, num1:, :]
            self.v[: self.batch_size, :, :diff, :] = value[:, :, num1:, :]
    ...
    def _prefill(
        ...
        self.k = self.k.to(key.dtype)
        self.v = self.v.to(value.dtype)
        self.k[:batch_size, :, :eff_init_length, :] = key[:, :, -eff_init_length:, :]
        self.v[:batch_size, :, :eff_init_length, :] = value[:, :, -eff_init_length:, :]
    ...
    def token_positions(self) -> torch.Tensor:
        return (
            self.token_pos[: self.current_length].reshape(1, 1, -1).expand(self.batch_size, self.n_query_groups, -1)
        )
